...
- **Bulk question import** (`POST /questions/import`) — upload an **Excel
  workbook** (one question per row: `statement`, `answer`, `optionA`…`optionH`,
  plus optional `type`/`difficulty`/`marks`/`explanation`/`tags`) or a **Word
  document** (questions written out as prose, with any diagrams imported and
  attached). The format is identified from the file's contents, so a wrong
  extension or MIME type does not matter. `GET /questions/import/template`
  returns a filled-in workbook with worked examples for all three question
  types. Every row lands as a DRAFT for approval, and a row that fails
  validation is reported with its number while the rest of the file imports.
...
- **Students & batches** — invite individually, or bulk-import a whole batch
  from an **Excel workbook** (`name`, `email`) — CSV still works for anyone who
  prefers it. `GET /students/import/template` returns the workbook to fill in.
  Roll numbers are always generated by the platform and never read from the
  file, so a pasted column cannot collide with an existing candidate. Rows are
  invited individually, so a bad row is reported with its number while the rest
  of the file imports. Organize into programs → classes → batches.
