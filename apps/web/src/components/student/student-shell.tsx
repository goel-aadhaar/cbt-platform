    ...
    <NavDrawerProvider>
      <div className="flex h-screen overflow-hidden bg-admin-bg text-admin-ink [font-family:var(--font-hanken)]">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-16 shrink-0 items-center gap-2 border-b border-admin-line bg-white px-3 sm:gap-3 sm:px-6">
            <NavDrawerToggle className="text-admin-muted" />
            {/* On a phone only the page you are on survives: a full trail plus
              separators is wider than the viewport before the bell and the
              settings menu have had their turn. The parent levels are one tap
              away in the drawer. */}
            <nav
            ...
            >
              <Link
                href="/student"
                className="hidden font-semibold text-admin-muted hover:text-admin hover:underline sm:inline"
              >
                Codonmind Nexus
              </Link>
              {breadcrumb.map((crumb, i) => {
                ...
                return (
                  ...
                  >
                    <span className="hidden text-admin-line sm:inline">/</span>
                    {isCurrent || !href ? (
                      <span
