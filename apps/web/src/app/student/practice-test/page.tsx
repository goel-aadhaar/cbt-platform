...
/**
 * Practice Test (§ Product Structure) — teacher-managed, flexible practice.
 * Scoped to `kind: "ASSESSMENT"` throughout, the same exam kind this screen
 * used to call "Assessment"/"My Assessments" before the product rename.
 *
 * The one real behaviour change from that earlier screen: a completed
 * attempt's score is available THE MOMENT it is submitted (the backend
 * evaluates it inline — see AttemptsService.submit()), not once the whole
 * window closes. The cross-student leaderboard is what still waits for the
 * window to close, which is a separate screen this one links out to.
 */
export default function PracticeTestPage() {
  ...
  return (
    <StudentShell breadcrumb={["Practice Test"]}>
      <header className="mb-6">
        <h1 className="text-3xl font-bold tracking-[-0.6px] text-admin-ink">
          Practice Test
        </h1>
        <p className="mt-1 text-sm text-admin-muted">
          Set by your teachers, on their own schedule. Flexible — switch tabs
          freely, use your phone, no proctoring. You can enter the moment the
          window opens, and your result appears the moment you submit.
        </p>
      </header>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 flex items-center gap-1.5 text-sm text-admin-muted">
                        ...
                        {e.durationMinutes} min once started · window closes{" "}
        ...
        <div className="overflow-hidden rounded-xl border border-admin-line/40 bg-white shadow-[0_4px_10px_rgba(0,0,0,0.04)]">
          ...
          {loadingHistory ? (
            ...
            <p className="px-5 py-8 text-center text-sm text-admin-muted">
              Nothing completed yet — Practice Tests you&apos;ve submitted will
              show up here with your score right away.
            </p>
          ) : (
            completed.map((a) => (
              <Link
